...
#' ..
#' 
#' @param x \link[rpart]{rpart} objects
#' 
#' @param xnm ..
#' 
#' @param ... ..
...
rmd_.rpart <- function(x, xnm, ...) {
  ...
  y <- model_[[1L]]
  txt2 <- if (inherits(y, what = 'Surv')) {
    'Kaplan-Meier estimates and curves based on the partition branches are created by <u>**`R`**</u> package <u>**`survival`**</u>.'
  } # else nothing
